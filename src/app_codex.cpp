...
static void drawRightColumn() {
  ...
  if (s_state == ST_RECORDING) {
    ...
    char buf[24];
    if (s_Codex_age < 60)        snprintf(buf, sizeof(buf), "%ds", s_Codex_age);
    else if (s_Codex_age < 3600) snprintf(buf, sizeof(buf), "%dm", s_Codex_age / 60);
    else                          snprintf(buf, sizeof(buf), "%dh", s_Codex_age / 3600);
    extra = buf;
